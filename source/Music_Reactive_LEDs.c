...
#define SAMPLE_SIZE (1 << BITS_IN_SAMPLES) // 2^BITS_IN_SAMPLES = 8 samples
#define false 0
#define true 1
//#define bool uint8_t
...
void setup_ADC()
{
	...
	SIM->SCGC6 |= (1 << 27);
	ADC0->CFG1 = ADC_CFG1_ADICLK(0b00) | ADC_CFG1_MODE(0b10) | ADC_CFG1_ADIV(0b01);
	// Setup ADC
...
void set_rgb_color(int r, int g, int b) {
	setRed(r);   // Set the red LED duty cycle
	setGreen(g); // Set the green LED duty cycle
	setBlue(b);  // Set the blue LED duty cycle
	return;
}
...
int main(void)
{
	...
	while (1)
	{
		...
		if (delta < mean - mean / 6.6 && curr_time - colorChangeDelay > 350)
		{
			...
			if (delta > mean - mean / 4.4)
			{
				transitionColor(oldColor, rgb, 300); // Transition with inverted colors
			}
			else
			{
				delay_ms(150);
		...
		else if (delta > 300) //min threshold amplitude
		{
			float brightnessFactor = (sqrt((min(500, delta) - mean / 2)) / 22.3) * 255;
			uint8_t r = (red * brightnessFactor) / 255;
			uint8_t g = (green * brightnessFactor) / 255;
			uint8_t b = (blue * brightnessFactor) / 255;
